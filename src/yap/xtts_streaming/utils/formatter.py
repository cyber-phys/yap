...
from yap.xtts_streaming.utils.tokenizer import multilingual_cleaners
...
def format_audio_list(audio_files, target_language="en", whisper_model = "large-v3", out_path=None, buffer=0.2, eval_percentage=0.15, speaker_name="coqui"):
    audio_total_size = 0
    ...
    asr_model = WhisperModel(whisper_model, device=device)
    ...
    prefix_check = f"{out_path}/wavs/{audio_file_name_without_ext}_"
    ...
    # if True:
        ...
        for word_idx, word in enumerate(words_list):
            ...
            if word.word[-1] in ["!", "。",".","?"]:
                ...
                metadata["audio_file"].append(absoulte_path)
    ...
    print(f"length of combined: {len(combined_train_df_shuffled)}")
    num_val_samples = int(len(combined_train_df_shuffled) * eval_percentage)
    print(f"numer of val samples: {num_val_samples}")
