...
def train_gpt(custom_model,version, language, num_epochs, batch_size, grad_acumm, train_csv, eval_csv, output_path, max_audio_length=255995):
    ...
    # Define the path where XTTS files will be downloaded
    parent_output_path = os.path.dirname(os.path.dirname(output_path))
    CHECKPOINTS_OUT_PATH = os.path.join(parent_output_path, "base_models",f"{version}")
